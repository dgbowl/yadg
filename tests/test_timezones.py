...
def test_timestamp_parsing(datadir):
    ...
    assert (
        utc["data"][0]["timesteps"][0]["uts"] - cet["data"][0]["timesteps"][0]["uts"]
        == 7200
    ), "CEST should be 2 hours ahead of UTC"
    assert (
        utc["data"][0]["timesteps"][1]["uts"] - cet["data"][0]["timesteps"][1]["uts"]
        == 3600
...
def test_isotimestamp_parsing_utc(datadir):
    ...
    assert (
        dg["data"][0]["timesteps"][0]["uts"] == 1622557825.0
    ), "Z-suffix parsed to 'uts' correctly"
    assert (
        dg["data"][0]["timesteps"][1]["uts"] == 1622557825.0 + 7200 + 1
    ), "±HH:MM-suffix parsed to 'uts' correctly"
    assert (
        dg["data"][0]["timesteps"][2]["uts"] == 1622557825.0 + 2
...
def test_isotimestamp_parsing_cet(datadir):
    ...
    assert (
        dg["data"][0]["timesteps"][2]["uts"] == 1622557825.0 - 7200 + 2
