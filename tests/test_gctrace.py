...
@pytest.mark.parametrize("input, ts", [
  (
    { #ts1 - datasc parse & integration from file
      "folders": ["."], "prefix": "2019-12-03", "suffix": "dat.asc", 
      "parameters": {"tracetype": "datasc", "calfile": "gc_5890_FHI.json"}
    },{
      "nsteps": 1, "step": 0, "nrows": 5, 
      "method": "polyarc+TCD_PK_09b.met", "point": 1, 
    ...
    { #ts5 - fusion parse & integration from file
      "folders": ["."], "prefix": "", "suffix": "fusion-data", 
      "parameters": {"tracetype": "fusion", "calfile": "gc_fusion.json"}
    },{
      "nsteps": 1, "step": 0, "nrows": 5, 
      "method": "AS_Cal_20210702", "point": 0, 
    ...
    { #ts6 - fusion parse & integration from file
      ...
      "nsteps": 1, "step": 0, "nrows": 5, 
      "method": "AS_Cal_20210702", "point": 4, 
