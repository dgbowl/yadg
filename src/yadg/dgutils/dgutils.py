...
def pointdata(datagram, pars):
    ...
    # MCPT parameters
    Vc = np.pi * ufloat(*results["params"]["cavity"]["r"])**2 * \
         ufloat(*results["params"]["cavity"]["h"])
    Vs = np.pi * ufloat(*results["params"]["sample"]["r"])**2 * \
         ufloat(*results["params"]["sample"]["h"])
    delta = (ufloat(*results["params"]["sample"]["m"]) / \
            ufloat(*results["params"]["sample"]["rho"])) * (1 / Vs)
    c_i = pars["cavity"]["i"]
    s_i = pars["sample"]["i"]
    A = pars["mcpt"]["A"]
    B = pars["mcpt"]["B"]
    ...
    for section in datagram:
        ...
        if section["input"]["datagram"] == "qftrace":
            for p in section["results"]:
                ...
                s = conductivity.Qf2σ(Qs, Q0, fs, f0, Vs, Vc,
            ...
            for p in section["results"]:
                ...
                for key in ["CO", "methane", "CO2", "ethylene", "ethane",
                            "propylene", "propane", "butane", 
                            "acetic", "acrylic", "maleic"]:
                    r["xout"][key] = [p["FID"].get(key, {}).get("x", 0), 0, "%"]
                for key in ["O2", "N2"]:
                    r["xout"][key] = [p["TCD"].get(key, {}).get("x", 0), 0, "%"]
                xC3H8 = 100 * np.interp(p["uts"], [xp["uts"] for xp in xin],
                                       [xp["x"]["C3H8"] for xp in xin])
                xO2 = 100 * np.interp(p["uts"], [xp["uts"] for xp in xin],
                                       [xp["x"]["O2"] for xp in xin])
                if xC3H8 > 1.0:
