import pytest
from utils import datagram_from_input, standard_datagram_test, pars_datagram_test, datadir
...
@pytest.mark.parametrize("input, ts", [
  (
    { # ts1 - units on 2nd line, correct number of rows, correct value
      ...
      "nsteps": 1, "step": 0, "nrows": 6, "point": 0, 
    ...
    { # ts2 - timestamp from uts and index
      ...
      "nsteps": 1, "step": 0, "nrows": 6, "point": 2, 
    ...
    { # ts3 - semicolon separator, rtol for flow, but not for T
      ...
      "nsteps": 1, "step": 0, "nrows": 7, "point": 0, 
      "pars": {
    ...
    { # ts4 - semicolon separator, timestamp from timestamp and index
      ...
      "nsteps": 1, "step": 0, "nrows": 7, "point": 3, 
    ...
    { # ts5 - tab separator, sigma for T from atol
      ...
      "parameters": {
        "sep": "\t", 
        "timestamp": {"timestamp": {"index": 1, "format": "%d.%m.%Y %I:%M:%S%p"}}, 
        "sigma": {"T": {"atol": 0.05}}
      }
    },{
      "nsteps": 1, "step": 0, "nrows": 5, "point": 3, 
    ...
    { # ts6 - tab separator, timestamp from timestamp, format, index
      ...
      "nsteps": 1, "step": 0, "nrows": 5, "point": 4, 
    ...
    { # ts7 - tab separator, timestamp from timestamp, format, index
      ...
      "nsteps": 1, "step": 0, "nrows": 5, "point": 2, 
    ...
    { # ts8 - tab separator, timestamp from timestamp, format, index
      ...
      "nsteps": 1, "step": 0, "nrows": 5, "point": 0, 
    ...
    { # ts9 - tab separator, timestamp from iso date and iso time
      ...
      "nsteps": 1, "step": 0, "nrows": 4, "point": 1, 
    ...
    { # ts10 - timestamp from time with custom format only
      ...
      "nsteps": 1, "step": 0, "nrows": 3, "point": 0, 
    ...
    { # ts11 - timestamp from time with custom format only
      ...
      "nsteps": 1, "step": 0, "nrows": 3, "point": 1, 
    ...
    { # ts12 - convert functionality test
      ...
      "parameters": {
        ...
        "units": {"flow": "ml/min", "T": "K", "p": "atm"},
        "convert": {
          "flow": {"flow": {"calib": {"linear": {"slope": 1e-6/60}, "atol": 1e-8}}, "unit": "m3/s"}
        }
      }
    },{
      "nsteps": 1, "step": 0, "nrows": 7,  "point": 0, 
      "pars": {
        "flow": {"sigma": 0.0, "value": 15.0, "unit": "ml/min", "raw": True},
        "flow": {"sigma": 1e-8, "value": 2.5e-7, "unit": "m3/s", "raw": False}
      }
    }
  ),(
    { # ts13 - convert functionality with intercept only and both global and calib sigma
      ...
      "parameters": {
        "atol": 0.1,
        "convert": {
          "T": {"T": {"calib": {"linear": {"intercept": 273.15}, "atol": 0.5}}, "unit": "K"}
        }
      }
    },{
      "nsteps": 1, "step": 0, "nrows": 6, "point": 0, 
    ...
    { # ts14 - calfile functionality
      ...
      "nsteps": 1, "step": 0, "nrows": 6, "point": 0, 
      "pars": {"T": {"sigma": 0.1, "value": 296.25, "unit": "K", "raw": False}}
    }
  ),(
    { # ts15 - calfile functionality with fractions and total
      ...
      "nsteps": 1, "step": 0, "nrows": 1662, "point": 0, 
      "pars": {
        "C3H8": {"sigma": 0.0, "value": 0.0, "unit": "ml/min", "raw": False}, 
        "N2": {"sigma": 0.0, "value": 30.361, "unit": "ml/min", "raw": False}, 
        "O2": {"sigma": 0.0, "value": 1.579, "unit": "ml/min", "raw": False},
        "total": {"sigma": 0.030, "value": 31.940, "unit": "ml/min", "raw": False}
      }
    }
  ),(
    { # ts16 - calfile functionality with fractions and total
      ...
      "nsteps": 1, "step": 0, "nrows": 1662, "point": 100, 
      "pars": {
        "C3H8": {"sigma": 0.0, "value": 1.204, "unit": "ml/min", "raw": False}, 
        "N2": {"sigma": 0.0, "value": 35.146, "unit": "ml/min", "raw": False}, 
        "O2": {"sigma": 0.0, "value": 3.577, "unit": "ml/min", "raw": False},
        "total": {"sigma": 0.035, "value": 39.927, "unit": "ml/min", "raw": False}
      }
    }
  ),
])
